...
const countries = [
  ...
  'england','morocco','argentina','mexico',
  'poland','france','australia','denmark',
]
...
function Points({points}){
  ...
  <p className="text-center text-dark fw-bold fs-4">Points</p>
...
function Strikes({strikes}){
  ...
  <p className="text-center text-dark fw-bold fs-4 ">Strikes</p>
...
function App (){
  ...
  const [random, setRandom] = useLocalStorage('random', countries)
  ...
  function onGuessHandler (guess){
    if (guess.toLowerCase() === word.toLowerCase()){
      ...
      if(random.length > 1) {
        ...
        setMessage('Hooray!! You won the game!')
        setVariant('success')
        setDisplay(true)
        setActive(false)
      }
  ...
  function PassGuess(e){
    ...
    if(random.length > 1 && passes > 1){
      ...
      setMessage('Oh! You ran out of passes :(')
      setVariant('danger')
      setDisable(true)
      setDisplay(true)
    }
